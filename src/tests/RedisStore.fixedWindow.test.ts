...
describe("FixedWindowLimiter Integration", ()=>{
    ...
    it("allows the request untill the limit reached", async()=>{
        const limit=3;
    ...
    it("rejects the request after the limit reached ", async ()=>{
        const limit=3;
        const windowSizeMs=1000;

        for(let i=0; i<limit; i++){
            await store.executeFixedWindow("user-2", limit, windowSizeMs);

        }

        const rejected= await store.executeFixedWindow("user-2", limit, windowSizeMs);
